# FeBEx: Implementation Brief for Claude Code

> **Context for the coding agent**: This is a networking course project. Use the BMv2/Mininet template folder (FeBEx/ is already open, which contains common/ , networks/, and tasks/. Tasks/ can be modified accordingly, as it was previouslyt used for an assignment) that uses **finsy** (P4Runtime Python library) and **scapy** for packet crafting. Build FeBEx following the same patterns — same libraries, similar file organization, similar Makefile structure. Read the template thoroughly before starting.
