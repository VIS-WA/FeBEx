"""
FeBEx P4Runtime Controller — V2 (Sliding two-epoch window)
===========================================================
Identical to controller.py except epoch_rotation_loop writes
prev_epoch = N BEFORE advancing current_epoch to N+1.

This ensures the P4 data plane (febex_v2.p4) always has both the current
and previous epoch values, so duplicates that arrive just after a boundary
still match the prev_epoch check and are correctly suppressed.

Options: identical to controller.py
"""
...
async def epoch_rotation_loop(thrift_port: int, interval: float):
    """Increment current_epoch; write prev_epoch first (V2 sliding window)."""
    epoch = 0
    while True:
        await asyncio.sleep(interval)
        prev  = epoch
        epoch = (epoch + 1) & 0xFFFF
        # Write prev BEFORE current so the data plane never sees a gap
        write_register(thrift_port, "FeBExIngress.prev_epoch",    0, prev)
        write_register(thrift_port, "FeBExIngress.current_epoch", 0, epoch)
        logger.info("Epoch → %d (prev=%d)", epoch, prev)
